...
export default function Header(props) {
  const headerImages = useSelector((state) => state.images)
    .filter((image) => image.header)
    .map((image) => image.img);
  ...
  return (
    <Box>
      <Fade in={props.showHeader} timeout={2000}>
        <Stack direction={stackDirection} className="container" id="header-container">
          <Box id="header-left" width={boxWidth} sx={{ my: extraMargin }}>
            <Typography variant="h3">A NEW LINE OF NFTS FROM YOUR FAVORITE STREAMER</Typography>
